...
자체 지시사항을 추가하고 URL 또는 파일로 외부 지식을 제공할 수 있습니다 ([RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/) 방식). 봇은 애플리케이션 사용자 간에 공유할 수 있으며, 사용자 지정된 봇은 독립 실행형 API로도 게시할 수 있습니다 ([자세히 보기](./PUBLISH_API_ko-KR.md)).
...
관리자 대시보드에서 사용자/봇별 사용 현황을 분석할 수 있습니다. [자세히 보기](./ADMINISTRATOR_ko-KR.md)
...
- us-east-1 리전에서 [Bedrock 모델 액세스](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/modelaccess)로 이동 > `모델 액세스 관리` > `Anthropic / Claude 3` 전체, `Amazon / Nova` 전체, `Amazon / Titan Text Embeddings V2`, `Cohere / Embed Multilingual`을 모두 선택한 후 `변경 사항 저장`.
...
> [!Important]
> 선택적 매개변수를 설정하지 않으면 URL을 아는 누구나 가입할 수 있습니다. 프로덕션 환경에서는 보안 위험을 완화하기 위해 IP 주소 제한 및 자체 가입 비활성화를 강력히 권장합니다(회사 도메인의 이메일 주소만 가입할 수 있도록 allowed-signup-email-domains을 정의할 수 있음). ./bin 실행 시 ipv4-ranges와 ipv6-ranges로 IP 주소 제한을 설정하고, disable-self-register를 사용하여 자체 가입을 비활성화하세요.
...
AWS 관리형 서비스를 기반으로 구축된 아키텍처로, 인프라 관리의 필요성을 제거합니다. Amazon Bedrock을 활용하여 AWS 외부 API와의 통신 없이 확장 가능하고, 안정적이며, 보안성 있는 애플리케이션을 배포할 수 있습니다.
...
- [Amazon Bedrock 지식 기반](https://aws.amazon.com/bedrock/knowledge-bases/): 문서 임베딩 및 파싱 서비스를 제공하는 검색 증강 생성(RAG)을 위한 관리형 인터페이스 제공
...
초간단 배포는 [AWS CodeBuild](https://aws.amazon.com/codebuild/)를 사용하여 내부적으로 CDK로 배포를 수행합니다. 이 섹션에서는 CDK를 직접 사용하여 배포하는 절차를 설명합니다.
...
사용자는 사용자 지정 봇 생성 화면에서 [텍스트 생성 매개변수](https://docs.anthropic.com/claude/reference/complete_post)를 조정할 수 있습니다. 봇이 사용되지 않으면 [config.py](./backend/app/config.py)에 설정된 기본 매개변수가 사용됩니다.
...
이 샘플은 기본적으로 셀프 회원가입이 활성화되어 있습니다. 셀프 회원가입을 비활성화하려면 [cdk.json](./cdk/cdk.json)을 열고 `selfSignUpEnabled`를 `false`로 전환하세요. [외부 ID 제공자](#external-identity-provider)를 구성하면 이 값은 무시되고 자동으로 비활성화됩니다.

### 회원가입 이메일 주소 도메인 제한

기본적으로 이 샘플은 회원가입 이메일 주소의 도메인을 제한하지 않습니다. 특정 도메인의 회원가입만 허용하려면 `cdk.json`을 열고 `allowedSignUpEmailDomains`에 도메인 목록을 지정하세요.
...
`enableRagReplicas`는 [cdk.json](./cdk/cdk.json)의 옵션으로, Amazon OpenSearch Serverless를 사용하는 지식 기반의 데이터베이스 복제본 설정을 제어합니다.
...
- CloudFront를 사용자 지정 도메인을 사용하도록 구성
...
기능 개선 및 기타 개선 사항의 경우, **풀 리퀘스트를 생성하기 전에 구현 접근 방식과 세부 사항을 논의하기 위해 기능 요청 이슈를 먼저 생성해 주시면 감사하겠습니다. 버그 수정 및 언어 번역(i18n)의 경우 직접 풀 리퀘스트를 생성하세요.**
