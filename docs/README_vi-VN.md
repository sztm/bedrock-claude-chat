...
Một chatbot đa ngôn ngữ sử dụng các mô hình LLM do [Amazon Bedrock](https://aws.amazon.com/bedrock/) cung cấp cho AI sinh tạo.
...
> [!Lưu ý]
> Các giá trị ghi đè sẽ được hợp nhất với cấu hình cdk.json hiện có trong thời gian triển khai của AWS code build. Các giá trị được chỉ định trong phần ghi đè sẽ có ưu tiên hơn các giá trị trong cdk.json.
...
> [!Quan trọng]
> Nếu không gian lưu trữ trong môi trường cục bộ không đủ trong quá trình triển khai, việc khởi tạo CDK có thể dẫn đến lỗi. Nếu bạn đang chạy trên Cloud9 v.v., chúng tôi khuyến nghị mở rộng kích thước khối lượng của phiên bản trước khi triển khai.
...
Mẫu này có chức năng đăng ký tự động được bật theo mặc định. Để vô hiệu hóa đăng ký tự động, mở [cdk.json](./cdk/cdk.json) và chuyển `selfSignUpEnabled` thành `false`. Nếu bạn cấu hình [nhà cung cấp danh tính bên ngoài](#external-identity-provider), giá trị sẽ bị bỏ qua và tự động vô hiệu hóa.
...
Cảm ơn bạn đã cân nhắc đóng góp vào kho lưu trữ này! Chúng tôi hoan nghênh các bản sửa lỗi, bản dịch ngôn ngữ (i18n), cải tiến tính năng, [công cụ đại lý](./docs/AGENT.md#how-to-develop-your-own-tools) và các cải tiến khác.
...
[![Những người đóng góp của bedrock claude chat](https://contrib.rocks/image?repo=aws-samples/bedrock-claude-chat&max=1000)](https://github.com/aws-samples/bedrock-claude-chat/graphs/contributors)
